...
class ResidualDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def ddim_sample(self, x_input, shape, return_all=True):
        """
        DDIM sampling with optional intermediate outputs.
        Args:
            x_input: input tensor (condition or starting image)
            shape: (batch, C, H, W)
            return_all: if True, return all intermediate results as a list
                        else return only the final image
        """
        batch, device = shape[0], self.betas.device
        ...
        if self.condition:
            ...
            input_add_noise = img.clone()
        else:
            img = torch.randn(shape, device=device)
            input_add_noise = img.clone()

        x_start = None
        pred_type = "use_pred_noise"

        # 保存中间结果
        img_list = []
        if return_all:
            img_list.append(unnormalize_to_zero_to_one(input_add_noise))

        for time, time_next in time_pairs:
            time_cond = torch.full((batch,), time, device=device, dtype=torch.long)
            self_cond = x_start if self.self_condition else None

            preds = self.model_predictions(x_input, img, time_cond, 0, self_cond)
            x_start = preds.pred_x_start
            pred_res = preds.pred_res
            ...
            sqrt_factor = (betas2_cumsum_next - sigma2).sqrt() / betas_cumsum

            noise = 0 if eta == 0 else torch.randn_like(img)

            if pred_type == "use_pred_noise":
                img = img - alpha * pred_res - (betas_cumsum - (betas2_cumsum_next - sigma2).sqrt()) * pred_noise + sigma2.sqrt() * noise
            elif pred_type == "use_x_start":
                img = sqrt_factor * img + (1 - sqrt_factor) * x_start + \
                    (alpha_cumsum_next - alpha_cumsum * sqrt_factor) * pred_res + \
                    sigma2.sqrt() * noise
            elif pred_type == "special_eta_0":
                img = img - alpha * pred_res - (betas_cumsum - alpha_cumsum_next) * pred_noise
            elif pred_type == "special_eta_1":
    ...
    def sample(self, x_input=0, batch_size=16, last=True):
        ...
        return sample_fn(x_input, size, return_all=last)
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.results_folder='./results/test'
    ...
    def train(self,input,target,name):
        ...
        return self.test(input,name,last=True, FID=True)
    ...
    def test(self,input,file_name, sample=False, last=True, FID=False):
        ...
        if self.condition:
            ...
            with torch.no_grad():
                ...
                x_input_sample = input
 
                all_images_list = list(self.ema.ema_model.sample(
                    x_input_sample, batch_size=batches, last=True))
                one_list = [all_images_list[-1]]

                all_images = torch.cat(all_images_list, dim=0)  # 直接拼接所有 step 的结果
                print("Number of intermediate steps:", len(all_images_list))

                # 保存每个 step 的结果
                for i, img in enumerate(all_images_list):
                    utils.save_image(img, str(self.results_folder / f"{file_name}_step{i:03d}.png"))
            ...
            if last:
                ...
                nrow = one_list.shape[0]

            utils.save_image(one_list, str(
