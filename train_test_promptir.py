...
def main(opt):
    ...
    degeneration_model = Degradation(opt).to(device)
    
    ##############compute fisher

    if opt.compute_fisher:
        logger.info(f"-------------compute fisher to select the important params--------------")
        tta_model.compute_fisher(train_loader)
            

    for batch_idx, (de_id, degrad_patch,clean_patch) in tqdm(enumerate(train_loader), total=len(train_loader), desc="Training Progress"):
        degrad_patch= degrad_patch.cuda()
        clean_patch = clean_patch.cuda()

        # 执行 TTA
        # print("!!!!!!!!!300",degrad_patch.shape)

        logger.info(f"{batch_idx}")
        restored = tta_model(train_loader, degrad_patch, de_id, degeneration_model)
        

    logger.info(f"-------------保存模型--------------")
    # tta_model.save(batch_idx, ckpt_name="promptir_3task_tta.ckpt")

    logger.info(f"-------------开始推理与评估--------------")

    testset = PairedImageDataset(lq_dir, gt_dir)   # ← 新增部分
    ...
    for batch_idx, (de_id, degrad_patch, clean_patch) in tqdm(enumerate(test_loader), total=len(test_loader), desc="Evaluation Progress"):
        ...
        # 保存恢复图像
        output_path = '/data2/tn/code/work_tta/DCTTA/results/Rain100H_DCTTA/'
